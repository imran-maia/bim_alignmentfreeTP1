...
if __name__ == "__main__":
    ...
    print('--------------------------------')
    print("Computing sketches for each file")
    sketches = {}
    for filename in filenames:
        combined_seq = "".join(files[filename])  
        
        sketch = filter_smallest(combined_seq, k, s)  
        sketches[filename] = sketch
        sketches[filename].sort()


    print("Computing Jaccard similarity for all pairs of files (using sketches)")
    for i in range(len(filenames)):
        for j in range(i + 1, len(filenames)):
            jaccard = jaccard_from_sorted_lists(sketches[filenames[i]], sketches[filenames[j]])
            print(filenames[i], filenames[j], jaccard)